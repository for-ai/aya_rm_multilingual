...
def main():
    ...
    ############################
        ...
        model = model_builder(
            args.model, **model_kwargs, trust_remote_code=args.trust_remote_code
        )
        ...
        json.dump(
            {
                ...
                "extra_results": (
                    results_grouped if "reward-bench" in args.dataset else None
                ),
            },
            f,
        )
    ...
    if args.save_all:
        ...
        with open(output_path, "w") as f:
            for chosen, rejected in zip(scores_chosen, scores_rejected):
                f.write(
                    json.dumps({"chosen": scores_chosen, "rejected": scores_rejected})
                    + "\n"
                )
