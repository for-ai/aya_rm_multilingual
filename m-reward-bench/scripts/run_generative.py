...
def get_args():
    ...
    parser.add_argument("--vllm_openai_api_key", type=str, default=None, help="API key for vLLM OpenAI Compatible Server")
    
    parser.add_argument("--vllm_openai_base_url", type=str, default=None, help="base URL for vLLM OpenAI Compatible Server")
    
    parser.add_argument("--output_dir", type=str, required=True, help="Directory to save the results for custom dataset.")
...
def main():
    ...
    # vllm_openai_config
    vllm_openai_config = None
    if args.vllm_openai_base_url is not None and args.vllm_openai_api_key is not None:
        vllm_openai_config = {
            'vllm_openai_base_url': args.vllm_openai_base_url,
            'vllm_openai_api_key': args.vllm_openai_api_key,
        }
    ...
    is_api_models = is_api_models or (vllm_openai_config is not None)
    ...
    CUSTOM_DATASET = args.dataset_name is not None
    if CUSTOM_DATASET:
        ...
        def get_judgement(batch, debug=args.debug):
            ...
            if len(batch["text_chosen"]) <= 4:  # set up only for 1 or 2 turns
                winner, request, judgement = run_judge_pair(
                    prompt, answer_a, answer_b, args.model, multi_turn=mult_turn, model_modifier=model_modifier, vllm_openai_config=vllm_openai_config
                )
                if debug:
