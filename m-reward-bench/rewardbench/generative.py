...
def run_judge_pair(question, answer_a, answer_b, model, multi_turn=False, model_modifier=None, vllm_openai_config=None):
    ...
    if isinstance(model, list):
        ...
        judgments = []
        for m in model:
            winner, _, judgment = run_judge_pair(question, answer_a, answer_b, m, 
                                                 multi_turn, vllm_openai_config)
    ...
    if model in OPENAI_MODEL_LIST:
    ...
    elif vllm_openai_config is not None:        
        template = "chatgpt"
        conv = get_conv_template(template)
        ...
        judgment = chat_completion_openai(
            model, conv, temperature=0, max_tokens=100,
            api_dict=vllm_openai_config)
...
def chat_completion_openai(model, conv, temperature, max_tokens, api_dict=None):
    if api_dict is None:
        ...
        client = OpenAI(
            base_url=api_dict['vllm_openai_base_url'],
            api_key=api_dict['vllm_openai_api_key'],
        )
    output = API_ERROR_OUTPUT
    for _ in range(API_MAX_RETRY):
        try:
